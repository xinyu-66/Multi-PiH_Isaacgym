...
def extract_training_dataset(path, dataset_path, seed=0, store=True, draw_statistic=False):
        ...
        min_threshold = np.sort(distance_abs)[10]
        ...
        similar_index = np.where(distance_abs < min_threshold * 1.1)
...
if __name__ == "__main__":
        ...
        parser.add_argument('--seed', type=int, default=0, help='baseline of position and orientation')
        ...
        # extract_training_dataset(path=path, dataset_path=dataset_path, seed=seed, store=store, draw_statistic=draw_statistic) #
